...
#What my experiment does and why: My experiment looks to replicate the Implicit association task. The experiment starts by asking for the participant info, which is used to help
#organize the information and create a file name. The experiment shows the participant a word that is shown as a jpg.  The participant then has to click the left or right arrow depending on what category
#they think the word belongs in. This information is then stored with the response time in the csv file. 
#I picked this study because it has helped us understand unconscious biases and stereotypes and has helped researchers understand the ways in which these biases can influence behavior.
#=====================
#IMPORT MODULES
#=====================
...
block_start_text = visual.TextStim(win, text='Press left or right arrow key depending on which category you think the word falls under. %d.', color=(1, 1, 1), pos=(0, 0), height=36)
...
for iblock in range(nBlocks):
    ...
    #Save information regarding the trials
    block_n.append(block_i)
    trial_n.append(trial_i)
    participant_response.append(response)
    rt_list.append(rt)
    
win.close()# Close window and end experiment
#Make dataframe with the collected results
results = zip(block_n, trial_n,participant_response, rt_list)
results = pd.DataFrame(results, columns=['block','trial','rt'])
